...
function renderColleges() {
  ...
  collegesGrid.innerHTML = collegesData.map(college => `
    <div class="college-card brand-${college.brand} reveal">
      <div style="display: flex; gap: 1rem;">
        <div class="college-badge">${college.emoji}</div>
        <div class="college-info" style="flex: 1;">
          <h4>${college.name}</h4>
          <p style="font-size: 0.85rem; color: var(--secondary); margin: 0.25rem 0 0 0;">${college.location}</p>
        </div>
      </div>
      <div class="college-listings">
        <span class="listing-count">${college.listings}</span>
        <span class="listing-type">${college.types}</span>
      </div>
      <div class="college-actions">
        <button onclick="filterByCollege('${college.name}')">View Properties</button>
        <button onclick="saveCollege('${college.name}')">Follow</button>
      </div>
    </div>
  `).join('');
  
  collegeCount.textContent = `${collegesData.length} Colleges`;
}

function filterByCollege(collegeName) {
  console.log(`Filtering properties by college: ${collegeName}`);
  // Scroll to properties section
  const propertiesSection = document.getElementById('homes');
  if (propertiesSection) {
    propertiesSection.scrollIntoView({ behavior: 'smooth' });
  }
}

function saveCollege(collegeName) {
  console.log(`Followed: ${collegeName}`);
  alert(`Now following ${collegeName}!`);
}
